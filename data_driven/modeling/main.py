...
def modeling_pipeline(X_train, Y_train, model, model_params, return_model=False):
    ...
    regressor = defining_model(model, model_params)

    # Cross validation
    logger.info(f' Applying 5-fold cross validation for {model} model')
    cv_result = performing_cross_validation(regressor, X_train, Y_train)

    # Y randomization
    logger.info(f' Applying Y-Randomization for {model} model')
    y_randomization_error = y_randomization(regressor, X_train, Y_train)
    ...
    if return_model:
        ...
        return modeling_results, regressor
